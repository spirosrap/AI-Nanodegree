...
def recognize_ngram(models: dict, test_set: SinglesData,probs,BIC_guesses):
   ...
   """
    ...
    for sentence in test_set.sentences_index.values():
      ...
      fivewords = sorted(f,key=f.get,reverse=True)[:6]
      for k in permutations(fivewords, r=len(sen)):
        l = 0
        for j in range(len(k)):
          l += f[k[j]]
        try:
          sentenceLP = l + 13*lm.log_s(" ".join(k)) ## According to one studen in the forum 13 is the best hyperparameter
      ...
      if(words == []):
        words = BIC_guesses[len(guesses):len(guesses)+len(sen)] ## Fall back to BIC guesses
      probabilities.append(prob)  
      guesses += words
    return (probabilities,guesses)
